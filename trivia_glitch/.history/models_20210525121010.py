...
class Constants(BaseConstants):
    ...
    #Specify the amount of questions you want in each round
    PHASE_1_LENGTH = 5
    PHASE_2_LENGTH = 16
    PHASE_3_LENGTH = 16

    #Specify bonus for each stage
    bonus_values = dict(EZ = .05, KD = .05, FB = .15)

    # Amount of money given for correct answer in Round 1 and 2
    BONUS_VALUE_ONE = float(.05)
    
    # Amount of money given for correct answer in Round 3
    BONUS_VALUE_TWO = float(.15)
    
    name_in_url = 'trivia_main'
    
    #temp = '/Users/edefilippis/Dropbox (Harvard University)/oTree/data/trivia_main.yaml'
    # Read in .yaml file containing quiz quesitons
    with open(r'./data/trivia_main.yaml') as file:
        questions = yaml.load(file, Loader=yaml.FullLoader)
   
    for dicts in questions:
        dicts['type'] = dicts['label'].split('_')[0]

    phase_1_indices = [i for i, x in enumerate(questions) if "EZ" in x['label']]
    phase_2_indices = [i for i, x in enumerate(questions) if "KD" in x['label']]
    phase_3_indices = [i for i, x in enumerate(questions) if "FB" in x['label']]

    players_per_group = 2
    num_rounds = (PHASE_1_LENGTH + PHASE_2_LENGTH + PHASE_3_LENGTH)
...
class Player(BasePlayer):
    ...
    total_score = models.IntegerField(initial = 0.0)
