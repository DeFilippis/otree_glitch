...
class Leavable_Wait_Custom(WaitPage):
    template_name = 'know_your_partner_new/leavable_wait_page_ED.html'
    group_by_arrival_time = True
    

    def vars_for_template(self):
        import time
        time_of_arrival = self.participant.vars.get('arrival_time_on_leavable_wait_page')
        time_change = datetime.timedelta(minutes=15)
        time_of_end = time_of_arrival + time_change
        ...
        return dict(
            ...
            <p>After 15 minutes of waiting, you will be automatically exited from the game. You will be given a code that you should email to me at defilippis@gmail.com, which I will use to create a $2.00 compensation HIT for your time. <b> Please refresh the page if you are not automatically removed from the study after waiting 15 minutes </b>. Please note that if you arrived to this HIT over 20 minutes late, you will not be eligible for any payment, as mentioned during scheduling.</p>''',

            leavable_wait_page_warning = '''Please note that in the real game it's very important that you use the chat box to interact with your opponent as much as possible. Please do not multitask during this game as failing to submit a page before the timer runs out will automatically kick you from the study.''',

            #time_of_arrival = time.time() * 1000,
            time_of_arrival = time_of_arrival,
            # Have to convert this to integer, if decimal points time doesn't work
            #time_of_end = int(round((time_of_arrival + 900000)))
            time_of_end = int(round(time_of_end.timestamp())) * 1000
